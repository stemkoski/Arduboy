...
void drawTileMap(int map[WORLD_HEIGHT][WORLD_WIDTH]) 
{
  for (int y = 0; y < WORLD_HEIGHT; y++) 
    for (int x = 0; x < WORLD_WIDTH; x++) 
       Sprites::drawOverwrite(x * TILE_SIZE, y * TILE_SIZE, tiles, map[y][x]);
}
...
void playScreen()
{
    ...
    /*
    if ( arduboy.pressed(UP_BUTTON) )    { chomperY = chomperY - 1; }
    if ( arduboy.pressed(DOWN_BUTTON) )  { chomperY = chomperY + 1; }
    if ( arduboy.pressed(LEFT_BUTTON) )  { chomperX = chomperX - 1; }
    if ( arduboy.pressed(RIGHT_BUTTON) ) { chomperX = chomperX + 1; }
    
    
    if ( arduboy.pressed(UP_BUTTON) )    { chomperFrame = 3; }
    if ( arduboy.pressed(DOWN_BUTTON) )  { chomperFrame = 1; }
    if ( arduboy.pressed(LEFT_BUTTON) )  { chomperFrame = 2; }
    if ( arduboy.pressed(RIGHT_BUTTON) ) { chomperFrame = 0; }
    */
    ...
    else
    {
      ...
      int input = -1;
      if ( arduboy.pressed(UP_BUTTON) )    { input = 3; }
      if ( arduboy.pressed(DOWN_BUTTON) )  { input = 1; }
      if ( arduboy.pressed(LEFT_BUTTON) )  { input = 2; }
      if ( arduboy.pressed(RIGHT_BUTTON) ) { input = 0; }
      
      if ( input != -1 && bitRead( maze[chomperY/TILE_SIZE][chomperX/TILE_SIZE], input) == 0 )
      {
        // TODO: also check if valid move - if bitSet at this position is 0 (no wall); if not, break.
    ...
    // Sprites::drawOverwrite(chomperX, chomperY, chomper, chomperFrame);
